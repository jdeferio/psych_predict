...
output_filepath =  '/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/cdrn_ccw_grouped.csv'
...
cdrn_psych_hosp_bef_2013 = pd.read_csv('/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/cdrn_psych_hosp_bef_2013.csv', encoding='utf-8', names=['person_id'])

cdrn_psych_hosp_1315 = pd.read_csv('/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/cdrn_psych_hosp_1315.csv', encoding='utf-8', names=['person_id', 'visit_start_date', 'visit_concept_id', 'condition_type_concept_id'])
...
cdrn_psych_hosp_2014 = cdrn_psych_hosp_2014.sort_values(by=['person_id','visit_start_date'])
print("Length and Number of unique person_id's with psych hospitalization in 2014: ", len(cdrn_psych_hosp_2014),", ",cdrn_psych_hosp_2014.person_id.nunique())

# Creates var 'hosp_num' which is a successive count of psychiatric hospitalizations per 'person_id' (ascending order by date)
# Identifies the first psych hospitalization in 2014, but is not necessarily the first psych hosp ever
cdrn_psych_hosp_2014['hosp_num'] = cdrn_psych_hosp_2014.groupby('person_id').visit_start_date.apply(pd.Series.rank)

# Creates new df where only the first psychiatric hospitalization in 2014 is retained
cdrn_psych_hosp_2014_1st = cdrn_psych_hosp_2014[cdrn_psych_hosp_2014['hosp_num'] == 1.0]
...
# Creates a filters patients who had a previous hospitalization for a psychiatric condition
# id_list = []
...
# cdrn_psych_hosp_2014_1st_ = cdrn_psych_hosp_2014_1st_.drop(cdrn_psych_hosp_2014_1st_.columns[[1,2,3,4]], axis =1).drop_duplicates()
...
# Merges the 2014 psych hospitalization file with the CCW results
cdrn_1st_psych_dx = pd.merge(cdrn_psych_hosp_2014_1st_, cdrn_ccw_results, how='inner', on='person_id')
...
# Filters the resultant df so that only the dx that occurred on or before the first psych hospitalization are present
cdrn_1st_psych_dx = cdrn_1st_psych_dx[(cdrn_1st_psych_dx.condition_start_date < cdrn_1st_psych_dx.visit_start_date)]
print("Number of unique persons with psych hospitalization and dx history < hospitalization date:", cdrn_1st_psych_dx.person_id.nunique())


# # Reset the Index
# cdrn_psych_hosp_2014_1st_ = cdrn_psych_hosp_2014_1st_.reset_index(drop=True)
#
# psych_included = list(cdrn_all_individual_visits.person_id.values)
# psych_missing_dict = {"psych_missing":[]}
...
cdrn_no_psych_flag = pd.merge(cdrn_ccw_results,psych_id, how='left', on='person_id')
...
# feat = ['person_id']
# no_psych = cdrn_no_psych_dx.loc[:,feat]
# no_psych = no_psych.drop_duplicates()
# print(len(no_psych))
# pd.DataFrame(no_psych).to_csv('/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/no_psych_hosp.csv', encoding='utf-8')


# cdrn_psych_hosp_missing = pd.read_csv('/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/cdrn_psych_ccw_results.csv', encoding='utf-8')
...
cdrn_hosp_util_2014 = pd.merge(cdrn_psych_hosp_2014_1st_.drop(cdrn_psych_hosp_2014_1st_.columns[[2,3,4]], axis=1),cdrn_all_visits_2014, how='left', on='person_id')

cdrn_hosp_util_2014 = cdrn_hosp_util_2014[(cdrn_hosp_util_2014.condition_start_date < cdrn_hosp_util_2014.visit_start_date)]

cdrn_no_hosp_util_2014 = pd.merge(cdrn_no_psych_dx.drop(cdrn_no_psych_dx.columns[1:], axis =1).drop_duplicates(), cdrn_all_visits_2014, how='left', on='person_id').fillna(0)
