...
output_filepath =  '/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/cdrn_ccw_grouped_prevent.csv'
...
cdrn_prevent_hosp_bef_2014 = pd.read_csv('/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/cdrn_prevent_hosp_bef_2014.csv', encoding ='utf-8', names=['person_id'])

cdrn_prevent_hosp_2014 = pd.read_csv('/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/cdrn_prevent_hosp_2014.csv', encoding ='utf-8', names=['person_id', 'visit_start_date', 'visit_concept_id'])
...
cdrn_prevent_hosp_2014 = cdrn_prevent_hosp_2014.sort_values(by=['person_id','visit_start_date'])
print("Length and Number of unique person_id's with any preventable hospitalization in 2014: ", len(cdrn_prevent_hosp_2014),", ",cdrn_prevent_hosp_2014.person_id.nunique())

# Creates var 'hosp_num' which is a successive count of hospitalizations per 'person_id' (ascending order by date)
# Identifies the first preventable hospitalization in 2014, but is not necessarily the first hosp ever
cdrn_prevent_hosp_2014['hosp_num'] = cdrn_prevent_hosp_2014.groupby('person_id').visit_start_date.apply(pd.Series.rank)

# Creates new df where only the first hospitalization in 2014 is retained
cdrn_prevent_hosp_2014_1st = cdrn_prevent_hosp_2014[cdrn_prevent_hosp_2014['hosp_num'] == 1.0]
print("Length of all preventable hospitalizations in 2014: ", len(cdrn_prevent_hosp_2014_1st))

# Reset Index
cdrn_prevent_hosp_2014_1st = cdrn_prevent_hosp_2014_1st.reset_index(drop=True)

# Creates a filters patients who had a previous hospitalization
# id_list = []
# for _ in cdrn_prevent_hosp_2014_1st['person_id']:
#     id_list.append(_)
# print("Length id_list:", len(id_list))

prev_hosp = []
for _ in cdrn_prevent_hosp_2014_1st['person_id']:
    if _ in(set(list(cdrn_prevent_hosp_bef_2014['person_id']))):
        prev_hosp.append(1)
...
cdrn_hosp_flag = pd.merge(cdrn_prevent_hosp_2014_1st,first_hosp_id, left_index=True, right_index=True)
print("Length cdrn_hosp flag:",len(cdrn_hosp_flag))


# Remove Patients that had a previous preventable hospitalization
cdrn_prevent_hosp_2014_1st_ = cdrn_hosp_flag[cdrn_hosp_flag['id'] == 0]

# cdrn_prevent_hosp_2014_1st_ = cdrn_prevent_hosp_2014_1st_.drop(cdrn_prevent_hosp_2014_1st_.columns[[1,2,3,4]], axis =1).drop_duplicates()
print("Number of unique persons who do NOT have a preventable hospitalization prior to 2014: ", len(cdrn_prevent_hosp_2014_1st_), cdrn_prevent_hosp_2014_1st_.person_id.nunique())
pd.DataFrame(cdrn_prevent_hosp_2014_1st_).to_csv('/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/cdrn_prevent_hosp_2014_1st_.csv', encoding='utf-8')

# Merges the 2014 preventable hospitalization file with the CCW results
cdrn_1st_hosp_dx = pd.merge(cdrn_prevent_hosp_2014_1st_, cdrn_ccw_results, how='inner', on='person_id')
print("Number of unique persons with a preventable hospitalization and dx / visit history:", cdrn_1st_hosp_dx.person_id.nunique())
print("Length of cdrn_1st_hosp_dx:", len(cdrn_1st_hosp_dx))


# Filters the resultant df so that only the dx that occurred on or before the first preventable hospitalization are present
cdrn_1st_hosp_dx = cdrn_1st_hosp_dx[(cdrn_1st_hosp_dx.condition_start_date < cdrn_1st_hosp_dx.visit_start_date)]
print("Number of unique persons with a preventable hospitalization and dx history < hospitalization date:", cdrn_1st_hosp_dx.person_id.nunique())


# # Reset the Index
# cdrn_prevent_hosp_2014_1st_ = cdrn_prevent_hosp_2014_1st_.reset_index(drop=True)
#
...
#
# # Merges df psych_missing with the preventable hospitalizations
...
# print("Missing preventable hospitalizations: ", len(cdrn_psych_hosp_2014_1st_edit))
#
...
# Creates a df of 'person_id' for people who had a preventable hospitalization before end of 2014
cdrn_all_hosp = cdrn_prevent_hosp_2014.person_id.append(cdrn_prevent_hosp_bef_2014.person_id).drop_duplicates()
...
# Removes patients with a previous preventable hospitalization
cdrn_no_hosp_dx = cdrn_no_hosp_flag[cdrn_no_hosp_flag['id'] != 1]
print("Number of patients with no preventable hospitalizations: ", cdrn_no_hosp_dx.person_id.nunique())

feat = ['person_id']
no_prevent = cdrn_no_hosp_dx.loc[:,feat]
no_prevent = no_prevent.drop_duplicates()
print(len(no_prevent))
pd.DataFrame(no_prevent).to_csv('/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/no_prevent_hosp.csv', encoding='utf-8')
...
# ========================================================================= #
#
#     START CREATION OF 2014 UTILIZATION METRICS FOR PREVENTABLE HOSP PTS
#
# ========================================================================= #

cdrn_hosp_util_2014 = pd.merge(cdrn_prevent_hosp_2014_1st_.drop(cdrn_prevent_hosp_2014_1st_.columns[[2,3,4]], axis=1),cdrn_all_visits_2014, how='left', on='person_id')
...
def util_convert(df):
    output_filepath2 = '/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/cdrn_2014_util_vars_prevent.csv'
...
util2014_ = pd.read_csv('/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/cdrn_2014_util_vars_prevent.csv', encoding='utf-8')
...
pd.DataFrame(cdrn_util2014_grouped).to_csv('/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/cdrn_util2014_prevent_grouped.csv', encoding='utf-8')
