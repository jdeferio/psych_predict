...
cdrn = pd.read_csv('/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/cdrn_ccs_modified_label_primary.csv', encoding='utf-8')
...
def data_processing(df):
    ...
    procs = pd.read_csv('/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/cdrn_procs_grouped_1218.csv',
                        encoding='utf-8', index_col=False)
    # procs = procs.drop(procs.columns[0], axis=1)

    # Removes patients no diagnosis history prior to hospitalization / end of 2014
    df = df.loc[(df['age'] >= 18) & (df['visit_count'] >= 2) & (df['date_diff'] >= 30)]
    df = df.loc[
        (df['mood'] > 1) | (df['schizo'] > 1) | (df['personality'] > 1) | (df['anxiety'] > 1) | (df['adjustment'] > 1)]
    ...
    case = cdrn.loc[cdrn.psych_hosp == 1, :]
    control = cdrn.loc[cdrn.psych_hosp != 1, :]
    # for i in range(1,100):
    ctrl_sample = control.sample(n=len(case))
    cdrn_match = case.append(ctrl_sample, ignore_index=True)

    X = cdrn_match.loc[:, cdrn_match.columns != 'psych_hosp']
    y = cdrn_match.loc[:, cdrn_match.columns == 'psych_hosp']
...
def lr_model(x1, y1):
    ...
    logreg = LogisticRegression(penalty='l1', C=1, solver='liblinear', class_weight='balanced', random_state=42,
                                n_jobs=-1)
...
def rf_model(x1, y1):
    ...
    # {'bootstrap': False, 'max_depth': 80, 'max_features': 'sqrt', 'min_samples_leaf': 1, 'min_samples_split': 5, 'n_estimators': 1600}

    rf = RandomForestClassifier(bootstrap= False, max_depth= 80, max_features='sqrt', min_samples_leaf= 1, min_samples_split= 5, n_estimators= 1600, n_jobs=1)
    rf.fit(X_train, np.ravel(y_train))

    imp = importances(rf, X_test, y_test)  # permutation
    imp = imp.reset_index()
    imp_ = imp[imp['Importance'] >= 0.0013]
...
def rf_model2(x1, y1, ft):
    ...
    # grid_search.fit(X_train, np.ravel(y_train))
    # print("== RF Best Params ==", grid_search.best_params_, '\n')
    #{'bootstrap': True, 'max_depth': None, 'max_features': 'sqrt', 'min_samples_leaf': 2, 'min_samples_split': 8, 'n_estimators': 2000} 

    rf = RandomForestClassifier(bootstrap= True, max_depth= None, max_features= 'sqrt', min_samples_leaf= 2, min_samples_split= 8, n_estimators= 2000, oob_score=True, n_jobs=-1)
    ...
    viz1.save('/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/Psych_RF_feat_depend_primary_sub_.png')
    ...
    viz2.save('/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/Psych_RF_feat_imp_primary_sub.png')
    ...
    plt.savefig('/Users/jdeferio/Documents/CCS_ROC_Psych_RF_primary_sub.png')
    plt.savefig('/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/CCS_ROC_Psych_RF_primary_sub.png')
    plt.show()
...
def xgb_model2(x1, y1, ft):
    ...
    D = feature_dependence_matrix(X_train)
    viz1 = plot_dependence_heatmap(D, figsize=(11, 10))
    viz1.save('/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/Psych_XGB_feat_depend_primary_sub.png')
    ...
    imp = importances(xgb1, X_test, y_test)  # permutation
    viz2 = plot_importances(imp)
    viz2.save('/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/Psych_XGB_feat_imp_primary_sub.png')
    ...
    plt.legend(loc="lower right")
    plt.savefig('/Users/jdeferio/Documents/ROC_Psych_XGB_primary_sub.png')
    plt.savefig('/Users/jdeferio/PycharmProjects/CDRN_Individual/venv/ROC_Psych_XGB_primary_sub.png')
    plt.show()
