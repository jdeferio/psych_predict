...
# IMPORT DATA
case_control = pd.read_csv("data/cdrn_psych_casecontrol2.csv", encoding="utf-8")
...
def ccs_aggregate(df):
    ...
    cdrn_ccs_grouped = reduce(
        lambda left, right: pd.merge(left, right, on="person_id"), dfs
    )
...
def dx_convert(df):
    ...
    visits = pd.read_csv("data/psych_casecontrol_visit.csv", encoding="utf-8")
...
def hosp_label(df):
    cdrn_psych_hosp_bef_18 = pd.read_csv(
        "data/cdrn_psych_hosp_bef_18.csv",
        encoding="utf-8",
        names=[
            "person_id",
            "inpt_hosp_date",
    ...
    for _ in df["person_id"]:
        if _ in (list(cdrn_psych_hosp_bef_18["person_id"])):
            ...
            psych_hosp_list_final.append(0)

    person_psych = pd.DataFrame(
        {"person_id": person_list2, "psych_hosp": psych_hosp_list_final}
    )
