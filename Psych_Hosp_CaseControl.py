...
)  # cdrn_psych_primary_hosp_bef_18.csv
diagnosis = pd.read_csv(
    "data/cdrn_psych_dx_bef_18.csv",
    encoding="utf-8",
    names=["person_id", "psych_dx_date"],
)
visits = pd.read_csv(
    "data/cdrn_all_visits_1218.csv",
    encoding="utf-8",
    names=["person_id", "visit_occurrence_id", "visit_start_date", "visit_concept_id"],
)
no_hosp = pd.read_csv(
    "data/cdrn_no_psych_hosp_bef_18.csv",
    encoding="utf-8",
    names=["person_id", "condition_start_date", "condition_source_value"],
)
demographics = pd.read_csv(
    "data/cdrn_pt_demographics.csv",
    encoding="utf-8",
    names=["person_id", "gender_concept_id", "birth_date"],
)
...
def control_find(w, x, z):
    ...
    control = x.groupby("person_id").apply(
        lambda y: y.sample(n=1, random_state=1).reset_index(drop=True)
    )
    ...
    for _ in control["condition_start_date"]:
        if _ < dt.datetime(2013, 1, 1, 00, 00, 00):
            ...
            dt.datetime(2013, 1, 1, 00, 00, 00)
            <= _
            <= dt.datetime(2016, 12, 31, 00, 00, 00)
        ):
            ...
            delta = (
                dt.datetime(2017, 12, 31, 00, 00, 00) - _
            ).days  # update to 12.31.2017 with larger sample size
            if delta == 0:
                d2 = _
                d1 = d2 - np.timedelta64(366, "D")
                dates["visit_win_end"].append(d2)
    ...
    control_3 = control_3[
        (control_3.visit_start_date >= control_3.visit_win_start)
        & (control_3.visit_start_date < control_3.visit_win_end)
    ]
    ...
    control_4_["date_diff"] = control_4_.apply(
        lambda row: days_between(str(row["first_y"]), str(row["last_y"])), axis=1
    )
    ...
    control_5.rename(
        columns={"visit_win_end": "index_date", "visit_win_start": "one_year_date"},
        inplace=True,
    )

    control_6 = control_5.merge(w, on="person_id", how="inner")
    control_6["age"] = control_6.apply(
        lambda row: round(
            (days_between(str(row["last_y"]), str(row["birth_date"])) / 365)
        ),
        axis=1,
    )
...
def case_find(w, x, y, z):
    ...
    df = df[
        df["inpt_hosp_date"] > "2012-12-31"
    ]  # selects only those patients w/ first hosp after 2012

    df = df.merge(y, on="person_id", how="inner")
    df["one_year_date"] = df["inpt_hosp_date"].values.astype(
        "datetime64[D]"
    ) - np.timedelta64(366, "D")
    cases = df[
        (
            df.psych_dx_date.values.astype("datetime64[D]")
            >= df.one_year_date.values.astype("datetime64[D]")
        )
        & (
            df.psych_dx_date.values.astype("datetime64[D]")
            < df.inpt_hosp_date.values.astype("datetime64[D]")
        )
    ]
    ...
    cases_2 = cases_2[
        (
            cases_2.visit_start_date.values.astype("datetime64[D]")
            >= cases_2.one_year_date.values.astype("datetime64[D]")
        )
        & (
            cases_2.visit_start_date.values.astype("datetime64[D]")
            < cases_2.inpt_hosp_date.values.astype("datetime64[D]")
        )
    ]

    cases_3 = cases_2.groupby("person_id").visit_start_date.agg(["first", "last"])
    cases_3 = cases_3.reset_index()
    cases_3["date_diff"] = cases_3.apply(
        lambda row: days_between(str(row["first"]), str(row["last"])), axis=1
    )
    ...
    cases_5["age"] = cases_5.apply(
        lambda row: round(
            (days_between(str(row["last"]), str(row["birth_date"])) / 365)
        ),
        axis=1,
    )
    cases_5["enc"] = cases_5.apply(
        lambda row: 1 if row.condition_type_concept_id == 44786627 else 2, axis=1
    )
...
def case_control(df1, df2):
    ...
    pd.DataFrame(df4).to_csv("data/cdrn_psych_casecontrol2.csv", encoding="utf-8")
